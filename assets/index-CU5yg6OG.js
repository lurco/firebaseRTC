 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */registerVersion(name,version,"app");function jsSnippetToJson(jsSnippet){const noComments=jsSnippet.replace(/\/\/.*$|\/\*[\s\S]*?\*\//gm,""),noTrailingCommas=noComments.replace(/,(\s*[}\]])/g,"$1"),obj=eval("("+noTrailingCommas+")"),jsonString=JSON.stringify(obj);return JSON.parse(jsonString)}let servers=null,firebaseConfig=null,app=null,db=null,localStream=null,remoteStream=null;const urlParams=new URLSearchParams(window.location.search),fconfig=urlParams.get("fconfig");function initializeFirebase(){app=initializeApp(firebaseConfig),db=getFirestore(app)}fconfig?(firebaseConfig=jsSnippetToJson(urlParams.get("fconfig")),initializeFirebase()):console.log("No Firebase config found in URL, put it in fconfig query parameter.");function createPc(){const n=new RTCPeerConnection(servers);window.pc=n,n.ontrack=e=>{e.streams[0].getTracks().forEach(t=>{remoteStream.addTrack(t)})},n.onconnectionstatechange=e=>{n.connectionState==="connected"&&(console.log("Connected!"),remoteVideo.srcObject=remoteStream)},n.ondatachannel=e=>{const t=e.channel;t.onmessage=r=>{console.log("Received Message:",r.data)},t.onopen=()=>{console.log("Chat Channel Opened!")},t.onclose=()=>{console.log("Chat Channel Closed!")}},n.oniceconnectionstatechange=e=>console.log("ICE connection state change:",e),window.stream=remoteStream}const webcamButton=document.getElementById("webcamButton"),webcamVideo=document.getElementById("webcamVideo"),callButton=document.getElementById("callButton"),callInput=document.getElementById("callInput"),answerButton=document.getElementById("answerButton"),remoteVideo=document.getElementById("remoteVideo"),hangupButton=document.getElementById("hangupButton");document.getElementById("initialize");document.getElementById("peer-connection-config");const initForm=document.getElementById("init-form");initForm.addEventListener("submit",n=>{n.preventDefault(),servers=jsSnippetToJson(n.target.elements["peer-connection-config"].value),createPc()});webcamButton.onclick=async()=>{localStream=await navigator.mediaDevices.getUserMedia({video:!0,audio:!0}),remoteStream=new MediaStream;const n=pc.createDataChannel("chat");n.onmessage=e=>{console.log("Received Message:",e.data)},n.onopen=()=>{console.log("Chat Channel Opened!")},n.onclose=()=>{console.log("Chat Channel Closed!")},localStream.getTracks().forEach(e=>{pc.addTrack(e,localStream)}),webcamVideo.srcObject=localStream,callButton.disabled=!1,answerButton.disabled=!1,webcamButton.disabled=!0};callButton.onclick=async()=>{const n=collection(db,"calls"),e=doc(n,"webrtc"),t=collection(e,"offerCandidates"),r=collection(e,"answerCandidates");await setDoc(e,{offer:null,answer:null}),await getDocs(t).then(a=>{a.docs.forEach(u=>{deleteDoc(u.ref)})}),callInput.value=e.id,pc.onicecandidate=async a=>{a.candidate&&await addDoc(t,a.candidate.toJSON())};const s=await pc.createOffer();await pc.setLocalDescription(s);const o={sdp:s.sdp,type:s.type};await updateDoc(e,{offer:o}),onSnapshot(e,a=>{const u=a.data();if(!pc.currentRemoteDescription&&(u!=null&&u.answer)){const h=new RTCSessionDescription(u.answer);pc.setRemoteDescription(h)}}),onSnapshot(r,a=>{a.docChanges().forEach(u=>{if(u.type==="added"){const h=new RTCIceCandidate(u.doc.data());pc.addIceCandidate(h)}})}),hangupButton.disabled=!1};answerButton.onclick=async()=>{const n=callInput.value,e=collection(db,"calls"),t=doc(e,n),r=collection(t,"answerCandidates"),s=collection(t,"offerCandidates");pc.onicecandidate=async d=>{d.candidate&&await addDoc(r,d.candidate.toJSON())};const a=(await getDoc(t)).data();console.log(a.offer),await pc.setRemoteDescription(new RTCSessionDescription(a.offer));const u=await pc.createAnswer();await pc.setLocalDescription(u);const h={type:u.type,sdp:u.sdp};await updateDoc(t,{answer:h}),onSnapshot(s,d=>{d.docChanges().forEach(m=>{if(m.type==="added"){let P=m.doc.data();pc.addIceCandidate(new RTCIceCandidate(P))}})})};
